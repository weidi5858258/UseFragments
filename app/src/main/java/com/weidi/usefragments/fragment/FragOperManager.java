...
public class FragOperManager implements Serializable {
    ...
    public int changeFragment(BaseFragment curUsedFragment,
                              BaseFragment needToShowFragment) {
        if (curUsedFragment == null
                || needToShowFragment == null
                || mCurUsedActivity == null
                || mCurUsedMainFragment == null
                || mActivityContainersMap == null
                || mActivityContainersMap.isEmpty()
    ...
    private int pop_back_stack_scene_1(
            FragmentManager fManager,
            FragmentTransaction fTransaction,
            final Fragment shouldPopFragment) {
        List<Fragment> parentFragmentsList = mActivityFragmentsMap.get(mCurUsedActivity);
        if (parentFragmentsList == null
        ...
        if (!shouldPopFragment.isHidden()) {
            fManager = mCurUsedActivity.getFragmentManager();
            fTransaction = fManager.beginTransaction();
            fTransaction.hide(shouldPopFragment);
            fTransaction.commit();
        }
        ...
        if (count < 1) {
            ...
            fTransaction.remove(shouldPopFragment);
            mUiHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    onDestroyAndOnDetach(shouldPopFragment);
                }
            }, 50);
            fTransaction.commit();
            /***
